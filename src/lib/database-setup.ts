...
// Removido import de @/lib/db pois não está disponível no contexto do servidor
...
export class DatabaseSetup {
  ...
  constructor(config: DatabaseSetupConfig) {
    this.pool = new Pool({
      host: config.host,
      port: config.port,
  ...
  private async createExtensions(client: any): Promise<void> {
    await client.query(`CREATE EXTENSION IF NOT EXISTS "uuid-ossp"`)
  }

  private async createEnums(client: any): Promise<void> {
  ...
  private async createForeignKeys(client: any): Promise<void> {
    const foreignKeys = [
      // Abastecimentos
      {
  ...
  private async createFunctionsAndTriggers(client: any): Promise<void> {
