...
// Tipos para associações de frota ativas para CT-e
export interface AssociacaoFrotaAtivaCTe {
  id: string;
...
// Função para obter associações ativas para CT-e
export async function getAssociacoesAtivasParaCTe(): Promise<AssociacaoFrotaAtivaCTe[]> {
  try {
  ...
  // Mapear os dados com estrutura aninhada
  const associacoesMapeadas = result.rows.map((row: any) => {
    ...
    const associacao: AssociacaoFrotaAtivaCTe = {
      ...row,
      funcionario: {
        nome: row.funcionario_nome,
        matricula: row.funcionario_matricula,
